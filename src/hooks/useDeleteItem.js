        ...
        () => {
            ...
            if (item?.name) {
                /* 
                    ...
                    needs to take in. 
                    From the apiItem, the del function takes in the item.id, user.token, & source.token (cancelToken)(Must be in the order they are in 
                ...
                */
                (async ()=>{
                    response = await apiItem.delItem(item.id, user.token, source.token)
                    if (response) {
                        setAlert({msg:`Item: ${item.name} has been deleted`,'cat':'success'})
